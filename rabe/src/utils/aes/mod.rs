...
pub fn encrypt_symmetric<G: std::convert::Into<Vec<u8>>>(_msg: G, _plaintext: &Vec<u8>) -> Result<Vec<u8>, RabeError> {
    ...
    encrypt_symmetric_with_rng(_msg, _plaintext, &mut rng)
}

/// Key Encapsulation Mechanism (AES-256 Encryption Function)
pub fn encrypt_symmetric_with_rng<G, R>(_msg: G, _plaintext: &Vec<u8>, rng : &mut R) -> Result<Vec<u8>, RabeError> 
where
    G: std::convert::Into<Vec<u8>>,
    R: Rng + CryptoRng,
{
